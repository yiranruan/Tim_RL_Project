...
class Maze:
    ...
    def add_pacman_stuff(self, max_food=2, toskip=0):
        ...
        #     row = 1
        #     col = 2
        #     if (row > self.r-6) and (col < 6):
        #         continue
        #     if(abs(col - self.c/2) < 3):
        #         continue
    ...
    def addFood(self, max_food):
        ...
        # fixed
        # self.food_list.append(food((5, 5)))
        # self.food_pos.append((5, 5))
        # self.grid[5][5] = R
        # self.food_list.append(food((5, 8)))
        # self.food_pos.append((5, 8))
        # self.grid[5][8] = C
    ...
    def getRealFood(self):
        foodGrid = copy_grid(self.grid)
        x, y = self.food_list[0].get_pos()
        for i in range(len(self.grid)):
            for j in range(len(self.grid[i])):
                if i == x and j == y:
                    foodGrid[i][j] = '1'
    ...
    def getFakeFood(self):
        foodGrid = copy_grid(self.grid)
        x, y = self.food_list[1].get_pos()
        for i in range(len(self.grid)):
            for j in range(len(self.grid[i])):
                if i == x and j == y:
                    foodGrid[i][j] = '1'
    ...
    #         if (pos == self.food_list[i].get_pos()) and self.food_list[i].get_status():
    #             return True
...
def generateMaze(length, width, seed=None):
    # if not seed:

...
class DeepQNetwork:
    ...
    def _build_net(self, rows, cols):
        length = rows + 2
        width = cols + 2
        # self.image = tf.placeholder(tf.float32, [length, width], name='Image')
    ...
    def store_transition(self, image, a, r, image_):
        ...
        transition = np.hstack((observation,[a,r], observation_))
    ...
    def save_network(self, train, name):
        ...
        if train:
            ...
            self.saver.save(self.sess, '/home/yiranruan/state_s/data/checkpoint_dir_'+name+'/MyModel_'+name)
        else:
            # saver = tf.train.import_meta_graph('/Workfiles/python3/Tim_RL_Project/checkpoint_dir/MyModel.meta')
            self.saver.restore(self.sess, tf.train.latest_checkpoint('/home/yiranruan/state_s/data/checkpoint_dir_'+name))
